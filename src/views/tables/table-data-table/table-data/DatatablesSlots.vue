...
<template>
  ...
  <div>
    <v-list-item-subtitle class="text-wrap">
      The
      <code>v-data-table</code> provides a large number of slots for customizing the table. This example showcases some of these slots and what you can do with each. It is important to note some slot (eg:
      <code>item</code>/
      <code>body</code>/
      <code>header</code>) will completely takes over the internal rendering of the component which will require you to re-implement functionalities such as selection and expansion. Some slots will override each other such as:
      <code>body</code> >
      <code>item</code> >
      <code>item.name</code> and
      <code>header</code>/
    ...
    <div class="mt-4">
      ...
      >
        ...
        <template v-if="isEnabled('no-data')" v-slot:no-data>NO DATA HERE!</template>

        <template v-if="isEnabled('no-results')" v-slot:no-results>NO RESULTS HERE!</template>
